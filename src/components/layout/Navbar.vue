    ...
    <header>
        ...
        <nav :class="[
            `fixed inset-0 z-20 flex flex-col items-center justify-center 
             bg-primary md:relative md:justify-between md:flex-row 
             ${isMenuOpen ? 'block' : 'hidden'} md:flex md:bg-transparent`
        ]">
        <ul class="flex flex-col items-center space-y-5 md:flex-row md:space-x-5 md:space-y-0">
            <li v-for="item in Menu" :key="item.name">
                <a :href="item.href" class="block transition ease-linear md:text-lg lg:text-xl font-bold text-white
                 md:text-primary hover:text-secondary
                  dark:text-white dark:hover:text-secondary" @click="scrollToSection(item.href)">{{ item.name }}</a>
            </li>
        </ul>
